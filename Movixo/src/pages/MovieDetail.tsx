...
import { Star, Clock, Calendar, ChevronLeft, MapPin, Play, Loader2, AlertCircle } from 'lucide-react';
import { moviesApi, showsApi } from '../lib/api';
import { adaptMovie, adaptTheatreWithShows } from '../lib/dataAdapter';
...
export default function MovieDetail() {
  ...
  const { selectedCity, setCurrentBooking } = useAppContext();
  ...
  useEffect(() => {
    if (!id) return;
    moviesApi.getReviews(Number(id)).then(setReviews).catch(() => {});
  }, [id]);
  ...
  useEffect(() => {
    ...
    setShowsError(null);
    showsApi
      .getByMovieAndDate(Number(id), selectedDate, selectedCity)
      .then((data) =>
        setTheatres(data.map((d) => adaptTheatreWithShows(d.theatre, d.shows)))
      )
      .catch((err) => setShowsError(err.message))
      .finally(() => setLoadingShows(false));
  }, [id, selectedDate, selectedCity]);

  const handleShowSelect = (theatre: Theatre, show: Theatre['shows'][0]) => {
    setCurrentBooking({ movie: movie!, theatre, show, date: selectedDate });
    navigate('/seat-selection');
  };

  const getAvailabilityColor = (available: number, total: number) => {
    if (available === 0) return 'text-red-400 bg-red-400/10';
    if (available / total < 0.2) return 'text-orange-400 bg-orange-400/10';
    ...
    <div className="min-h-screen bg-[#0f0f1a]">
      ...
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="flex gap-6 -mt-24 relative z-10 mb-8">
          ...
          <div className="flex-1 pt-20 md:pt-24">
            <div className="flex flex-wrap items-center gap-2 mb-2">
              <span className="bg-[#e63946] text-white text-xs font-bold px-2 py-0.5 rounded">{movie.certificate}</span>
              {movie.format.map((f) => (
                <span key={f} className="bg-white/10 text-white text-xs px-2 py-0.5 rounded border border-white/20">{f}</span>
              ))}
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3 mb-3">
              ...
              <span className="text-gray-400 text-sm">{movie.votes.toLocaleString()} votes</span>
            ...
            <div className="flex flex-wrap gap-2">
              {movie.genre.map((g) => (
                <span key={g} className="bg-white/5 border border-white/10 text-gray-300 text-xs px-3 py-1 rounded-full">{g}</span>
              ))}
              {movie.language.map((l) => (
                <span key={l} className="bg-white/5 border border-white/10 text-gray-400 text-xs px-3 py-1 rounded-full">{l}</span>
              ))}
            </div>
          </div>
        </div>
        ...
        <div className="flex gap-1 mb-6 bg-white/5 p-1 rounded-xl w-fit">
          {(['about', 'cast', 'reviews'] as const).map((tab) => (
            <button
              key={tab}
              onClick={() => setActiveTab(tab)}
              className={`px-5 py-2 rounded-lg text-sm font-semibold capitalize transition-all ${
                activeTab === tab ? 'bg-[#e63946] text-white shadow-lg' : 'text-gray-400 hover:text-white'
              }`}
            >
              {tab}
                ...
                <div className="bg-white/5 border border-white/10 rounded-xl p-4">
                  <div className="grid grid-cols-2 gap-4">
                    {[
                      ...
                      { label: 'Release', value: new Date(movie.releaseDate).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' }) },
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
              {movie.cast.map((member) => (
                <div key={member.name} className="bg-white/5 border border-white/10 rounded-xl p-4 text-center hover:border-[#e63946]/30 transition-all">
                  <div className="w-16 h-16 rounded-full bg-gradient-to-br from-[#e63946]/30 to-purple-600/30 border-2 border-white/10 mx-auto mb-3 overflow-hidden flex items-center justify-center">
                    {member.image ? (
                  ...
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-gray-500 text-xs">{new Date(review.createdAt).toLocaleDateString('en-IN')}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-1 bg-green-500/20 px-2 py-1 rounded">
                      ...
                      <span className="text-green-400 text-xs font-bold">{review.rating}/10</span>
                    </div>
                  </div>
        ...
        <div className="border-t border-white/10 pt-8">
          ...
          {/* Theatres */}
          {!loadingShows && !showsError && (
            <div className="space-y-4 mb-12">
              {theatres.length === 0 && (
                <div className="text-center py-12">
                  <p className="text-gray-400">No shows available for this date in {selectedCity}.</p>
                </div>
              )}
                  ...
                  <div className="flex items-start justify-between mb-4">
                    ...
                    <div className="flex gap-2">
                      {theatre.amenities.slice(0, 2).map((a) => (
                        <span key={a} className="text-xs bg-white/5 border border-white/10 text-gray-400 px-2 py-0.5 rounded">{a}</span>
                      ))}
                    </div>
                  </div>
                  <div className="flex flex-wrap gap-3">
                    {theatre.shows.map((show) => (
                      <button
                        ...
                        className={`group flex flex-col items-center border rounded-xl px-4 py-3 transition-all ${
                          show.availableSeats === 0
                            ? 'border-red-500/20 bg-red-500/5 cursor-not-allowed opacity-60'
                            : 'border-white/10 bg-white/5 hover:border-[#e63946]/50 hover:bg-[#e63946]/10 cursor-pointer'
                        }`}
                      >
                        ...
                        <div className={`mt-1.5 text-xs px-2 py-0.5 rounded-full font-medium ${getAvailabilityColor(show.availableSeats, show.totalSeats)}`}>
                          {getAvailabilityLabel(show.availableSeats)}
                        </div>
