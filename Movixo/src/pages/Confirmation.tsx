...
export default function Confirmation() {
  ...
  if (!currentBooking?.movie || !currentBooking?.bookingId) {
    return (
    ...
    <div className="min-h-screen bg-[#0f0f1a] flex items-center justify-center px-4 py-12">
      <div className="w-full max-w-md">
        ...
        <motion.div
          ...
          className="bg-[#16213e] rounded-2xl overflow-hidden border border-white/10 shadow-2xl"
        >
          {/* Movie Banner */}
              ...
              <div className="flex items-start gap-2">
                ...
                <div>
                  ...
                  <p className="text-white text-sm font-semibold">
                    {date ? new Date(date).toLocaleDateString('en-IN', { weekday: 'short', day: 'numeric', month: 'long' }) : ''}
                  </p>
                </div>
              </div>
              <div className="flex items-start gap-2">
                ...
                <div>
                  <p className="text-gray-500 text-xs">Show Time</p>
                  <p className="text-white text-sm font-semibold">{show?.time}</p>
                </div>
              </div>
              <div className="flex items-start gap-2 col-span-2">
                <MapPin className="w-4 h-4 text-[#e63946] mt-0.5 shrink-0" />
                <div>
                  <p className="text-gray-500 text-xs">Theatre</p>
                  <p className="text-white text-sm font-semibold">{theatre?.name}</p>
        ...
        >
          <button className="flex-1 flex items-center justify-center gap-2 bg-white/10 hover:bg-white/15 border border-white/10 text-white py-3 rounded-xl text-sm font-semibold transition-all">
          ...
          <button
            onClick={() => navigate('/')}
            className="flex-1 flex items-center justify-center gap-2 bg-[#e63946] hover:bg-[#c1121f] text-white py-3 rounded-xl text-sm font-semibold transition-all"
          >
