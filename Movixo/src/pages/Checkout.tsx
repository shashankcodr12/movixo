...
import { bookingsApi, promoApi } from '../lib/api';
import { adaptBookingResponse } from '../lib/dataAdapter';
...
export default function Checkout() {
  ...
  if (!currentBooking?.movie || !currentBooking?.seats) {
    return (
  ...
  // Validate promo code against Java backend
  ...
  // Submit booking to Java backend
  const handlePay = async () => {
    ...
    try {
      const seatIds = seats!.map((s) => {
        // seatId is the label like "A1"; backend maps label → DB id via show
        // If your backend uses numeric IDs, store them in Seat.id as string
        return parseInt(s.id.replace(/\D/g, ''), 10) || 0;
      });
  ...
  const paymentOptions = [
    { id: 'upi' as PaymentMethod, label: 'UPI', icon: Smartphone, desc: 'Pay via UPI ID or QR code' },
    { id: 'card' as PaymentMethod, label: 'Credit/Debit Card', icon: CreditCard, desc: 'Visa, Mastercard, RuPay' },
    { id: 'netbanking' as PaymentMethod, label: 'Net Banking', icon: Building2, desc: 'All major banks' },
    { id: 'wallet' as PaymentMethod, label: 'Wallets', icon: Wallet, desc: 'Paytm, PhonePe, Amazon Pay' },
  ];
        ...
        {!isLoggedIn && (
          <div className="mb-6 flex items-center gap-3 bg-yellow-500/10 border border-yellow-500/30 rounded-xl p-4">
            ...
            <p className="text-yellow-300 text-sm">
              You need to{' '}
              <button onClick={() => navigate('/login')} className="underline font-semibold">sign in</button>
          ...
          <div className="lg:col-span-3 space-y-4">
              ...
              >
                <div className="flex items-center gap-3">
                  <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 ${
                    paymentMethod === option.id ? 'border-[#e63946]' : 'border-gray-500'
                  }`}>
                    ...
                    {option.id === 'upi' && (
                      <div>
                        ...
                        <input
                          ...
                          className="w-full bg-white/10 border border-white/20 text-white placeholder-gray-500 px-4 py-2.5 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#e63946]"
                        />
                      </div>
                    )}
                    {option.id === 'card' && (
                      <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          <div>
                            <label className="text-gray-400 text-xs mb-1.5 block">Expiry</label>
                            <input
                              type="text"
                              placeholder="MM/YY"
                              value={cardExpiry}
                    ...
                    {option.id === 'netbanking' && (
                      <div className="grid grid-cols-3 gap-2">
                        {['SBI', 'HDFC', 'ICICI', 'Axis', 'Kotak', 'PNB'].map((bank) => (
                          <button key={bank} className="bg-white/10 hover:bg-white/20 border border-white/10 text-white text-sm py-2 rounded-lg transition-all">
                            {bank}
                          </button>
                        ))}
                      </div>
                    ...
                    {option.id === 'wallet' && (
                      <div className="grid grid-cols-3 gap-2">
                        {['Paytm', 'PhonePe', 'Amazon Pay', 'Mobikwik', 'Freecharge', 'Ola Money'].map((w) => (
                          <button key={w} className="bg-white/10 hover:bg-white/20 border border-white/10 text-white text-xs py-2 px-2 rounded-lg transition-all">
                            {w}
                          </button>
                        ))}
                      </div>
            ...
            <div className="bg-white/5 border border-white/10 rounded-xl p-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-white font-medium text-sm">Promo Code</span>
              </div>
          ...
          <div className="lg:col-span-2">
            <div className="bg-white/5 border border-white/10 rounded-xl overflow-hidden sticky top-24">
              ...
              <div className="p-4 space-y-2">
                ...
                {promoApplied && (
                  <div className="flex justify-between text-sm">
                    <span className="text-green-400">Discount ({promoCode})</span>
