          ...
          <div className="relative aspect-[2/3] overflow-hidden">
            ...
            <div className="absolute bottom-2 left-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
              {movie.format.slice(0, 2).map((f) => (
                <span key={f} className="bg-[#e63946]/80 text-white text-xs px-1.5 py-0.5 rounded font-medium">
                  {f}
                </span>
              ))}
            </div>
          ...
          <div className="p-3">
            ...
            <div className="mt-2 flex flex-wrap gap-1">
              {movie.language.slice(0, 2).map((lang) => (
                <span key={lang} className="text-xs bg-white/5 text-gray-400 px-2 py-0.5 rounded-full border border-white/10">
                  {lang}
                </span>
              ))}
            </div>
